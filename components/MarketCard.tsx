...
export function MarketCard({ market, onBet, style }: MarketCardProps) {
  ...
  const handleBet = useCallback((side: "YES" | "NO") => {
    ...
    setFlashSide(side);
    setTimeout(() => setFlashSide(null), 500);
    ...
    <div
      className={cn(
        ...
        market.isHot
          ...
          : "market-card-default border-white/[0.08]"
      )}
      style={style}
    >
      {/* Card Top */}
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-white/35">{market.kolHandle}</div>
          </div>
        </div>
      ...
      <div className="flex gap-1.5 mb-2.5">
        <button
          ...
          className={cn(
            ...
            "bg-green-400/[0.08] text-green-400 border-green-400/25 hover:bg-green-400/[0.18] active:scale-95",
            flashSide === "YES" && "bet-flash ring-1 ring-green-400"
          )}
        >
          YES {market.yesPct}%
        </button>
        <button
          ...
          className={cn(
            "flex-1 py-2 rounded-lg text-[12px] font-medium border transition-all duration-150",
            "bg-red-400/[0.08] text-red-400 border-red-400/25 hover:bg-red-400/[0.18] active:scale-95",
            flashSide === "NO" && "bet-flash ring-1 ring-red-400"
          )}
        >
          NO {market.noPct}%
        </button>
      ...
      <div className="flex justify-between text-[11px] text-white/30">
        ...
        {market.startsIn ? (
          <span className="text-orange-400 font-medium">{market.startsIn}</span>
        ) : time ? (
          <span className="text-orange-400 font-medium">{formatTime(time)}</span>
        ) : null}
      </div>
    </div>
  );
